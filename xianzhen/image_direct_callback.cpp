...
int main(void) {
    ...
    if (stDeviceList.nDeviceNum == 2) 
    {
        for (int i = 0; i < stDeviceList.nDeviceNum; i++) 
        {
            ...
            if(!PrintDeviceInfo(pDeviceInfo,i))
                return 0;            
        }  
    } 
    ...
    */
	...
	string upperCamera_command_str = "get upperCameraLineHeight";
	char* upperCamera_command = (char*)upperCamera_command_str.c_str();
	redisReply* upperCamera_reply = redis_command_execution(listener,upperCamera_command,REDIS_REPLY_STRING);
	if(upperCamera_reply == NULL){
		logger_file << logging_generation("ERROR","Cannot get upperCameraLineHeight");
		return 0;
	}
	string upperCameraLineHeight = upperCamera_reply->str;
	cout << "upperCameraLineHeight:" << upperCameraLineHeight << endl;
	freeReplyObject(upperCamera_reply);
	
	string underCamera_str = "get underCameraLineHeight";
	char* underCamer_command = (char*)underCamera_str.c_str();
	redisReply* underCamer_reply = redis_command_execution(listener,underCamer_command,REDIS_REPLY_STRING);
	if(underCamer_reply == NULL){
		logger_file << logging_generation("ERROR","Cannot get underCameraLineHeight");
		return 0;
	}
	string underCameraLineHeight = underCamer_reply->str;
	cout << "underCameraLineHeight:" << underCameraLineHeight << endl;
	freeReplyObject(underCamer_reply);
	
	string exposurelevel_str = "get exposurelevel";
        char* exposurelevel_command = (char*)exposurelevel_str.c_str();
	redisReply* exposurelevel_reply = redis_command_execution(listener,exposurelevel_command,REDIS_REPLY_STRING);
	if(exposurelevel_reply == NULL)
	{
                cout << "exposurelevel:error"  << endl;
		logger_file << logging_generation("ERROR","Cannot get exposurelevel");
		return 0;
	}
	string exposurelevel = exposurelevel_reply->str;
	freeReplyObject(exposurelevel_reply);
	cout << "exposurelevel:" << exposurelevel << endl;
	
	
	string select_5_command_str = "select 5";
    char* select_5_command = (char*)select_5_command_str.c_str();
    redisReply* select_5_reply = redis_command_execution(listener,select_5_command,REDIS_REPLY_STRING);
    if(select_5_reply == NULL){
        cout << "Cannot select 5" << endl;
        logger_file << logging_generation("ERROR","Cannot select 5");
        return 0;
    }
    freeReplyObject(select_5_reply);
	string exposurelevelNumber_command_str = "get "+exposurelevel;

	char* exposurelevelNumber_command = (char*)exposurelevelNumber_command_str.c_str();
	redisReply* exposurelevelNumber_reply = redis_command_execution(listener,exposurelevelNumber_command,REDIS_REPLY_STRING);
	if(exposurelevelNumber_reply == NULL){
		logger_file << logging_generation("ERROR","Cannot get upperCameraLineHeight");
		return 0;
	}
	string exposurelevelNumber = exposurelevelNumber_reply->str;
	cout << "exposurelevelNumber1_hahaha:" << exposurelevelNumber << endl;
	freeReplyObject(exposurelevelNumber_reply);
	
	select_0_command_str = "select 0";
    select_0_command = (char*)select_0_command_str.c_str();
    select_0_reply = redis_command_execution(listener,select_0_command,REDIS_REPLY_STRING);
    if(select_0_reply == NULL){
        cout << "Cannot select 0" << endl;
        logger_file << logging_generation("ERROR","Cannot select 0");
        return 0;
    ...
    float right_exposure = strtof(exposurelevelNumber.c_str(),NULL);//strtof(current_exposure.c_str(),NULL);
	int upper_height = stoi(upperCameraLineHeight);	
	cout << "upperCamera,right_exposure:" << upper_height<<","<<right_exposure << endl;
    ...
    float left_exposure = strtof(exposurelevelNumber.c_str(),NULL);
	int under_height = stoi(underCameraLineHeight);
	if(work_order.zipper_num == "3" || work_order.zipper_num == "5")
	{
		upper_height =(upper_height)*40+1000; 
    ...
    do {
		...
		logger_file << logging_generation("INFO","machine_status:"+machine_status);
        if(machine_status == "off")
        {
            if(line_one.nCurHeight!=0)
            {
                cout<<"machine stop and recover line camera one"<<endl;
                line_one.clear();
            }
            if(line_two.nCurHeight!=0)
            {
                cout<<"machine stop and recover line camera two"<<endl;
                line_two.clear();
            }
            if(first_off) 
            {
